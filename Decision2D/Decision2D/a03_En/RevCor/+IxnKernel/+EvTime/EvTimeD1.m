...
properties (Dependent)
    ...
    cond % (trial, 1) % numeric vector of condition.
    d_cond % (trial, 1) % 1-based index of condition category
end
properties (Hidden)
    ch_
    ev_
...
methods
    function [ev, ch, S_filt, tr_incl, fr_incl] = get_ev_filtered(Ev, varargin)
        ...
        % S_filt : struct containing settings
        % tr_incl(tr,1) : logical
        % fr_incl{tr,1}(1,fr) : index
        %
        % OPTIONS:
...
%             'UniformOutput', false);
        ...
        for tr = 1:n_tr
            ...
            fr_incl1 = fr_incl1( ...
                (fr_incl1 >= 1) ...
                & (fr_incl1 <= size(ev0, 2)));
            ev{tr} = ev0(tr, fr_incl1);
        end
        
        S_filt.tr_incl = tr_incl;
        S_filt.fr_incl = fr_incl;
...
methods
    ...
    function evs = get_ev_summary_intervals(Ev, varargin)
        ...
        S_filt = Ev.get_S_filt_intervals(varargin{:});
        ...
        for i_itv = 1:n_itv
            ...
            ev1 = Ev.get_ev_filtered(C_filt1{:});
            evs{i_itv} = cellfun( ...
                @(ev11) nanmean(ev11, 2), ...
                ev1);
        end
...
methods
    ...
    function v = get.cond(Ev)
        v = Ev.get_cond;
    end
    function set.cond(Ev, v)
        Ev.set_cond(v);
    end
    function v = get_cond(Ev)
        v = Ev.cond_;
    end
    function set_cond(Ev, v)
        Ev.cond_ = v;
    end
    
    function v = get.d_cond(Ev)
        v = Ev.get_d_cond;
    end
    function set.d_cond(Ev, v)
        Ev.set_d_cond(v);
    end
    function v = get_d_cond(Ev)
        [~,~,v] = unique(Ev.cond);
    end
    function set_d_cond(Ev, v)
        if isempty(Ev.cond_)
        else
%             error('Cannot set d_cond when cond_ is not empty!');
            warning('Setting d_cond despite cond_ is not empty!');
        end
        Ev.cond_ = v;
    end
end
