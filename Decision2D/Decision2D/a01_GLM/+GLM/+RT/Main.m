classdef Main < GLM.RT.CommonWorkspace
    % Fit.Rt.Main
    %
...
%         X = bsxfun(@minus, X, nanmean(X));
    ...
    function v = get_VarNames(W)
        if W.n_dim_task == 2
            v = {'r', 'i', 'rxi', 'ai', 'rxai', 'ci', 'rxci', 'RT'};
        else
            v = {'r', 'i', 'rxi', 'ai', 'rxai', 'RT'};
        end
    end
...
methods
    ...
    function fs = get_file_fields0(W)
        fs = [
            W.get_file_fields0@GLM.RT.CommonWorkspace
            {
            'n_var', 'nv'
            }];
    end
