...
%         % DEBUG
%         % (motion_cond, color_cond, ch)
%         % shows (2,3,5) % ch: sep by motion ch, any color ch
%         disp('squeeze(any(any(p, 1), 4))');
%         disp(squeeze(any(any(p, 1), 4)))
%         
%         % shows (2,3,4) % ch: sep by color ch, any motion ch
%         disp('squeeze(any(any(p, 1), 5))');
%         disp(squeeze(any(any(p, 1), 5)))
    end
