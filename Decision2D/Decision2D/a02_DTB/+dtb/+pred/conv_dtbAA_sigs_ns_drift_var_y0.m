...
elseif isvector(diffusion)
    diffusion = diffusion(:);
    assert(size(diffusion, 1) == nd);
else
    assert(ismatrix(diffusion));
    assert(size(diffusion, 1) == nd);
end
...
assert(all(wtAliasUp(:) >= -1));
assert(all(wtAliasUp(:) <= 1));
assert(all(wtAliasDn(:) >= -1));
assert(all(wtAliasDn(:) <= 1));
...
for k=1:nt 
    ...
    if k == 1 || ...
        ...
        if isscalar(sigma)
            sigmaSq_cdf1 = sigmaSq_cdf;
        elseif isvector(sigma)
            sigmaSq_cdf1 = sigmaSq_cdf;
        else
            sigmaSq_cdf1 = repmat(sigmaSq_cdf(:,k)', [(ny+1), 1]);
        end
...
%         hold on;
    ...
    for dd = 1:nd
        U(:, dd) = max(conv(U(:, dd), kernel(:, dd), 'same'), 0);
    end
...
try
    D.up.pdf_t(k,:)=max(sum(U(ixAliasUp(k):end,:),1), 0); % sum(U(y>=Bup(k),:),1); % YK
    D.lo.pdf_t(k,:)=max(sum(U(1:ixAliasDn(k)  ,:),1), 0); % y<=Blo(k),:),1); % YK
catch err_dtb
...
%         U(ixAliasUp(k),:) = 0;
    ...
    % DEBUG
    try
        assert(all(D.up.pdf_t(k,:) >= 0));
        assert(all(D.lo.pdf_t(k,:) >= 0));
    catch err_dtb
        warning(err_msg(err_dtb));
        keyboard;
    end
