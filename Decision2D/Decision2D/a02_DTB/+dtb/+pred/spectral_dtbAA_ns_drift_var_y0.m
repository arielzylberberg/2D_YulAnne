...
for k=1:nt 
    ...
    if size(y0, 3) == 1 && k == 1 % When y0 is specified only for the beginning,
    ...
    % When y0 is specified for each time point,
    % Add y0 *after* convolution
    % it should not diffuse instantly 
    % - it should diffuse only after one step
    if size(y0, 3) > 1 
        U = U + y0(:,:,k);
    end
