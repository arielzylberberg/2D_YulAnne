...
%         W.Data.load_data; % In case excluding outlier runs
        
%         W.set_to_exclude_bins_wo_trials;
    end
%     function set_to_exclude_bins_wo_trials(W, to_exclude_bins_wo_trials)
%         % set_to_exclude_bins_wo_trials(W, to_exclude_bins_wo_trials)
%         if nargin >= 2
%             W.to_exclude_bins_wo_trials = to_exclude_bins_wo_trials;
%         end
%         
...
% 
%         data = W.get_data_pdf;
%         n_cond_ch = permute(sum(data, 1), [2, 3, 4, 5, 1]);
% 
%         n_cond_ch_training = n_cond_ch .* to_incl_cond_ch;
%         cond_ch1_to_exclude = sums(n_cond_ch_training, [2, 4]) <= thres;
%         cond_ch2_to_exclude = sums(n_cond_ch_training, [1, 3]) <= thres;
% 
%         W.cond_ch_to_exclude = ...
%             bsxfun(@or, cond_ch1_to_exclude, cond_ch2_to_exclude);
%     end
...
%         % Version before separating get_cond_ch_to_incl()
%         % cost_sep1(t, ch1, ch2, cond1, cond2)
%         cost_sep1 = reshape(cost_sep, siz0([1, 4, 5, 2, 3]));
%         
%         % cost_sum_across_t(cond1, cond2, ch1, ch2)
%         cost_sum_across_t = ...
%             permute(sum(cost_sep1), [4, 5, 2, 3, 1]);
%         
%         if ~isempty(W.to_exclude_bins_wo_trials)
%             % to_excl_cond_ch(cond1, cond2, ch1, ch2)
%             to_excl_cond_ch = W.cond_ch_to_exclude; 
% 
%             cost_sum_across_t(to_excl_cond_ch) = 0;
%         end
%         
%         to_incl_cond = W.get_to_incl_cond_training;
%         
%         cost_sum_across_ch = sums(cost_sum_across_t, [3, 4]);
%         cost = sum(vVec(cost_sum_across_ch(to_incl_cond)));
%         
%         if nargout >= 2
%             cost_sep = permute(reshape(cost_sep, ...
%                 siz0([1, 4, 5, 2, 3])), [1, 4, 5, 2, 3]);
...
methods
    ...
    function W = create_RT(W0, varargin)
        S0 = varargin2S(varargin, {
            ...
            'th0', []
            ...
%             % Set disper_kind = std to allow 
%             'disper_kind', 'std' % 'std'|'log10_fano'
            });
        ...
        switch S0.model
            case {'Ser', 'Par'}
                S = varargin2S(varargin, varargin2S({
                    'td_kind', S0.model
                    ...
                    'fix_bias_irr_1', S0.fix_irr_ixn
                    'fix_bias_abs_irr_1', S0.fix_irr_ixn
                    'fix_bias_irr_2', S0.fix_irr_ixn
                    'fix_bias_abs_irr_2', S0.fix_irr_ixn                    
                    }, S0));
                C = S2C(S);
...
%                     'fix_fano_2', true
            ...
            case {'min_sub', 'min_sup', 'min_sup15', 'min_sup125'}
                assert(all(isfield(S0, {'subj', 'parad'})));
                assert(strcmp(S0.parad, 'RT'));
                
                subj = S0.subj(1:2); % ignore bootstrap
                
                S = varargin2S(varargin2S({
                    'model', 'InhSliceFix'
                    'slprops0', ...
                        Data.Consts.slprops0.(S0.parad).(S0.model).(subj);
                    }, varargin), S0);
                C = S2C(S);
                W = W0.create_RT(C{:});
...
%                 S = varargin2S(S, S);
        ...
        if ~isempty(S0.th0)
            W.th0 = copyFields(W.th0, S0.th0);
            W.th = copyFields(W.th, S0.th0);
        end

        % Collapse bound late for prd=RT
        if isfield(W.th0, 'Dtb__Bound1__b_logitmean')
            logit_b_mean0 = logit(1/5);
        ...
        %% Kappa
        if ~isempty(W.fix_kappa) && W.fix_kappa
            n_dim = 2;
            for dim = 1:n_dim
                th1 = Fit.get_1D_fit(W.subj, dim, W.parad);
                k = th1.Dtb__Drift__k;
                
                name1 = sprintf('Dtb__Dtb%d__Drift__k', dim);
                if isfield(W.th, name1)
                    W.th.(name1) = k;
                    W.fix_to_th_(name1);
                else
                    name1 = sprintf('Dtb__Drift%d__k', dim);
                    if isfield(W.th, name1)
                        W.th.(name1) = k;
                        W.fix_to_th_(name1);
                    else                
                        error('Designate kappa''s name!');
                    end
                end
            end
...
%         % DEBUG: see if linearity holds
%         rt_pred_pdf = rt_pred_pdf ./ sums(rt_pred_pdf, [1, 4, 5]);

        if S.to_use_to_excl
            % Exclude those that are excluded from the fit
            to_excl = W.cond_ch_to_exclude;
        
            % Enforce symmetric to_excl, to make serial prediction
            % straight on the plot
    %         to_excl = to_excl ...
    %             | flip(flip(to_excl, 1), 3) ...
    %             | flip(flip(to_excl, 2), 4) ...
    %             | flip(flip(flip(flip(to_excl, 1), 3), 2), 4);
    ...
    %         to_excl = repmat( ...
    %             permute(W.cond_ch_to_exclude_jt, [5, 1, 2, 3, 4]), ...
    %             [W.nt, 1, 1, 1, 1]);
            rt_data_pdf(to_excl) = 0;
            rt_pred_pdf(to_excl) = 0;
        end
            
%         conds_x = unique(abs(W.Data.conds{S.dim_on_x}));
...
%         bml.plot.crossLine('NE', 0, {'--', 0.75 + [0 0 0]});
        hold off;
    ...
    function [h, hxe, hye, y, e] = plot_rt_vs_rt_unit(W, p, varargin)
        ...
        % DEBUG
%         cond_plot_x = 1:n_cond1;
%         cond_plot_y = 1:n_cond2;
...
%         n_cond_plot_x = round(max(cond_plot_x) / 2); % DEBUG
        ...
        % Take mean or variance first then average them
        % - an alternative to pooling probability?
%         switch S.yfun
%             case 'mean'
%                 [e0, y0] = bml.stat.sem_distrib(p, t);
%             case 'var'
%                 [e0, y0] = bml.stat.sev_distrib(p, t);
%         end
%         for cond_x = 1:n_cond_plot_x
%             for group_y = 1:n_group_y
%                 cond_x_incl = cond_plot_x == cond_x;
%                 cond_y_incl = bsxEq(cond_plot_y(:), S.group_y{group_y}(:)');
%                 
%                 
%                 cond_y_incl = find(cond_y_incl);
%                 
%                 y1 = nanmean(vVec(y0(1, cond_x_incl, cond_y_incl, :, :)));
%                 e1 = nanmean(vVec(e0(1, cond_x_incl, cond_y_incl, :, :)));
%                 
%                 e(cond_x, group_y) = e1;
%                 y(cond_x, group_y) = y1;
%             end
%         end
        ...
        for cond_x = 1:n_cond_plot_x
            for i_group = 1:n_group_y
                ...
                cond_y_incl = find(cond_y_incl);
%                 cond_y_incl = cond_y_incl(1); % DEBUG - remove in production
%                 cond_y_incl = cond_y_incl(end); % DEBUG - remove in production

%                 p1 = p(:, cond_x_incl, cond_y_incl, 1, 1); % DEBUG - remove in production
%                 p1 = p(:, cond_x_incl, cond_y_incl, 2, 2); % DEBUG - remove in production
