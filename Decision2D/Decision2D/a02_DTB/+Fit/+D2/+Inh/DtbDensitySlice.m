classdef DtbDensitySlice < Fit.D2.Inh.DtbDensityIndivJt
    % With one factor, slprop, control how the sliced time is allocated.
    % dim1 gets slprop, and dim2 gets 1-slprop, where slprop is in [0, 1].
    % drift and sigmaSq for each dim linearly scales 
    % with slprop and 1-slprop.
    %
    % 2017 YK wrote the initial version.
methods
    function W = DtbDensitySlice(varargin)
        if nargin > 0
            W.init(varargin{:});
        end
    end    
    function init(W, varargin)
        ...
        W.add_params({
            {'logit_slprop', logit(0.5), logit(0.04), logit(1 - 0.04)}
            });
    end
    function pred(W)
        % Scale drift and sigmaSq_fac
        slprop = invLogit(W.th.logit_slprop);
