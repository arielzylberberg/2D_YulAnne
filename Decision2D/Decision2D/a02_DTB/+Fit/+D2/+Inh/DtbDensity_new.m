...
methods
    function pred(W)
        ...
        for dim_1st = W.get_dim_1st_incl
            ...
            %%
            td_final{dim_1st} = W.calc_Td_final( ...
                td_together, unabs_together);
        end
        
        %%
        td_merged = W.calc_Td_merge_dim_1st(td_final);
...
methods
    ...
    function [td_pdf, unabs] = calc_Td_together_unit(W, dim, dim_1st)
        ...
        drift_fac = W.get_drift_fac_together;
        drift_fac = drift_fac(:, dim_1st);
        Drift = W.get_Drift(dim);
        drift_cond_t = Drift.get_drift_cond_t;
        drift_cond_t = drift_cond_t * drift_fac(dim);
...
%         if dim ~= dim_1st ...
%                 && ~ismember(W.to_slow_collapse{dim}, {'none', ''})
%             switch W.to_slow_collapse{dim}
%                 case 'ssq'
%                     n_dim = 2;
%                     o_dim = n_dim + 1 - dim;
%                     time_speed_factor = ...
%                         W.sigmaSq_fac_together(dim, o_dim);
%                 otherwise
%                     error('Unknown to_slow_collapse{%d}=%s', dim, ...
%                         W.to_slow_collapse{dim});
%             end
%         else
%             time_speed_factor = 1;
%         end
...
%             'time_speed_factor', time_speed_factor);
        ...
        % DEBUG
        if W.TO_DEBUG
            n_cond = size(drift_cond_t, 1);
            total_bef = ones(n_cond, 1);
        end
        ...
        %% DEBUG
        if W.TO_DEBUG
            debug_sum_td_aft = vVec(sums(td_pdf, [1, 3], true));
            debug_sum_unabs_aft = 0;
            % Since we are using sign rule, no need to consider unabs
            % in sum.
%                 ...
%                     vVec(sums(unabs(end,:,:), 2, true));

            total_aft = debug_sum_td_aft ...
                      + debug_sum_unabs_aft;

            % DEBUG: disabled temporarily
            try
                assert_isequal_within(total_bef, total_aft, ...
                    W.DEBUG_THRES, 'relative_tol', false);
                
                assert(~any(isnan(td_pdf(:))), 'NaN in td!');
                assert(~any(isnan(unabs(:))), 'NaN in unabs!');
            ...
            if W.VERBOSE_DEBUG == 2
                fprintf('calc_Td_together, dim %d, sum_unabs_aft\n', dim);
                disp(debug_sum_unabs_aft(:)');
            end
        end

        % Sign rule is already applied in calc_dtb.
        % Following is unnecessary.
        %
%             % Use sign rule to make td_pdfs sum to 1 within condition.
%             % Rationale : unabs_together is used only when td_together 
%             % has reached bound. The last bin will be minned out in 
%             % calc_Td_together_first anyway, but having it will preserve
%             % the sum to 1.
%             td_pdf(end,:,1) = ...
%                 td_pdf(end,:,1) + ...
%                 hVec(sum(unabs(end, y<0, :), 2));
%             
%             td_pdf(end,:,2) = ...
%                 td_pdf(end,:,2) + ...
%                 hVec(sum(unabs(end, y>0, :), 2));
...
%             if any(y == 0)
%                 ix_y_eq_0 = find(y == 0);
%                 assert(isscalar(ix_y_eq_0));
%                 
%                 for ch = 1:2
%                     td_pdf(end,:,ch) = ...
%                         td_pdf(end,:,ch) + ...
%                         hVec(unabs(end, ix_y_eq_0, :) / 2);
%                 end
%             end

        if W.TO_DEBUG
            total_td_aft_sign_sum = ...
                vVec(sums(td_pdf, [1, 3], true));
%                 % DEBUG - temporarily disabled
            try
                assert_isequal_within( ...
                    total_td_aft_sign_sum, total_bef, ...
                    W.DEBUG_THRES, 'relative_tol', false);
                
                assert(~any(isnan(total_td_aft_sign_sum(:))), ...
                    'NaN in td_pdf!');
    ...
    function sigmaSq = get_sigmaSq_together(W, dim, dim_1st)
        ...
        sigmaSq_fac = W.get_sigmaSq_fac_together;
        sigmaSq_fac = sigmaSq_fac(:, dim_1st);
        sigmaSq = sigmaSq_fac(dim);
    end
end
%% ----- calc_Td_together_final
methods
    function td_final = calc_Td_final(W, td_together, unabs_together)
        % td_together{dim}(t, cond12, ch_dim)
        % unabs_together{dim}(t, y, cond12)
        
        %% -- Input
        n_dim = 2; % CONST
        
        %% Prepare sums for sanity check 
%         if W.TO_DEBUG
%             % DEBUG: Check normalization
%             % {dim}(cond12) <- {dim}(t, cond12, ch)
%             for dim = n_dim:-1:1
%                 sum_td_within_cond_orig{dim} = ...
%                     sums(td_together{dim}, ...
%                         [W.dim_td_together.t, W.dim_td_together.ch], ...
%                         true);
%                     
%                 if W.VERBOSE_DEBUG == 2
%                     fprintf('sum_td_within_cond_orig{%d}\n', dim);
%                     disp(sum_td_within_cond_orig{dim});
%                 end
%             end
%         end
        ...
        %% td_ge_any_ch
        % {dim}(t, cond12)
        % = P(R_dim >= t) % Omitting "| cond12)" hereafter.
        for dim = n_dim:-1:1
            % td_together_any_ch{dim}(t, cond12)
            td_together_any_ch{dim} = sum(td_together{dim}, 3);
            td_ge_any_ch{dim} = cumsum(td_together_any_ch{dim}, ...
                1, 'reverse');
%             td_gt_any_ch{dim} = cat(1, ...
%                 td_ge_any_ch{dim}(2:end,:), ...
%                 zeros([1, size(td_ge_any_ch{dim}, 2)]));
        end
        
        %% td_first_ch
        % {dim}(t, ch_dim, cond12)
        % = P(R_dim=t, Ch_dim=ch_dim, R_o_dim >= t)
        for dim = n_dim:-1:1
            o_dim = 3 - dim;
            td_first_ch{dim} = bsxfun(@times, ...
                permute(td_together{dim}, [1, 3, 2]), ...
                permute(td_ge_any_ch{o_dim}, [1, 3, 2]));
        end
        
        %% unabs_first
        % {dim}(t, y, cond12)
        % = P(R_dim>t, Y_dim=y, R_o_dim>=t)
        for dim = n_dim:-1:1
            o_dim = 3 - dim;
            unabs_first{dim} = bsxfun(@times, ...
                unabs_together{dim}, ...
                permute(td_ge_any_ch{o_dim}, [1, 3, 2]));
        end
...
%         td_same_ch = bsxfun(@times, ...
%             permute(td_first_ch{1}, [1, 2, 4, 3]), ...
%             permute(td_first_ch{2}, [1, 4, 2, 3]));
...
%                 permute(td_first_ch{dim},   [1, 4, 2, 3]), ...
...
%             unabs_diff_ch{dim} = bsxfun(@times, ...
%                 permute(td_first_ch{dim},   [1, 4, 2, 3]), ...
%                 permute(unabs_first{o_dim}, [1, 2, 4, 3]));
%         end
        ...
        for dim = n_dim:-1:1
            ...
            td_diff_ch{dim} = W.calc_Td_alone_unit(o_dim, p0);
        end
