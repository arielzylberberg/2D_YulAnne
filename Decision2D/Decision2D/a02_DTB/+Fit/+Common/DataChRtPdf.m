...
function load_data(Dat)
    ...
    Dat.exclude_outlier_runs();
end
function exclude_outlier_runs(Dat)
    if Dat.to_excl_outlier_runs
        % Load the list of outlier runs
        C = S2C(bml.oop.copyprops(struct, Dat, 'props', {
            'subj'
            'parad'
            'task'
            }));
        
        FindOutliers = Fit.Check.FindOutlierRuns;
        C = varargin2C({
            'to_excl_outlier_runs', Dat.to_excl_outlier_runs
            }, C);
        bml.oop.varargin2props(FindOutliers, C);
        Dat.ix_run_to_excl = FindOutliers.load_mat;
    else
        Dat.ix_run_to_excl = [];
    end
end
% function set.to_excl_outlier_runs(Dat, v)
%     v0 = Dat.to_excl_outlier_runs_;
%     if ~isequal(v0, v)
%         Dat.to_excl_outlier_runs_ = v;
%         
%         Dat.exclude_outlier_runs();
% %         Dat.filt_ds();
%     end
% end
% function v = get.to_excl_outlier_runs(Dat)
%     v = Dat.to_excl_outlier_runs_;
% end
