...
%     
%     % to_include_last_frame: Inherited from Fit.D2.Common.Main
...
%             W.to_use_easiest_only_for_comparison = 0;

%             S_file = W.get_S_file;
%             S_file = bml.struct.orderfields(S_file, {
%                 'ef', 'ec', 'lf'}, 'last');
...
%     function [cond_ch_to_excl, v00] = get_cond_ch_to_exclude(W)
%         n_tr_cond_ch = permute(sum(W.Data.RT_data_pdf), ...
%             [2, 3, 4, 5, 1]);
%         
%         to_excl = n_tr_cond_ch <= W.to_exclude_bins_wo_trials_thres;
%         
%         v00 = W.get_cond_ch_to_exclude@Fit.D2.Common.Main;
% %         v00 = W.get_cond_ch_to_exclude@Fit.D2.Common.Main( ...
% %             W.to_use_easiest_only_for_fit);
%         
%         cond_ch_to_excl = to_excl | v00;
%         
%         %%
%         % Fixing problem where a validation condition is included
%         % with no training condition
%         cond_to_incl_train = W.get_to_incl_cond_training;
%         cond_to_incl_valid = W.get_to_incl_cond_validation;
%         
%         cond_ch_to_incl_train = bsxfun(@and, ...
%             cond_to_incl_train, ...
%             ~cond_ch_to_excl);
%         cond_ch_to_incl_valid = bsxfun(@and, ...
%             cond_to_incl_valid, ...
%             ~cond_ch_to_excl);
%         
%         
%         
%         cond_ch_to_excl_train = ~cond_ch_to_incl_train;
%         
%         v1 = all(all(cond_ch_to_excl_train, 2), 4);
%         v2 = all(all(cond_ch_to_excl_train, 1), 3);
%         cond_ch_to_excl = bsxfun(@or, cond_ch_to_excl, ...
%             bsxfun(@or, v1, v2));
%         
% %         %%
% %         cond_to_incl_valid = W.get_to_incl_cond_validation;
...
% %             cond_to_incl_valid, ...
% %             ~cond_ch_to_excl);
% %         cond_ch_to_excl_valid = ~cond_ch_to_incl_valid;
% %         
% %         v1 = all(all(cond_ch_to_excl_valid, 2), 4);
% %         v2 = all(all(cond_ch_to_excl_valid, 1), 3);
% %         cond_ch_to_excl = bsxfun(@or, cond_ch_to_excl, ...
% %             bsxfun(@or, v1, v2));
% 
% %         %%
% %         while any_change
% %             cond_ch_to_excl0 = cond_ch_to_excl;
% %             v1 = all(all(cond_ch_to_excl, 2), 4);
% %             v2 = all(all(cond_ch_to_excl, 1), 3);
% %             cond_ch_to_excl = bsxfun(@or, cond_ch_to_excl, ...
% %                 bsxfun(@or, v1, v2));
% %             any_change = ~isequal(cond_ch_to_excl0, cond_ch_to_excl);
% % %             fprintf('-');
% %         end
% % %         fprintf('\n');
%     end
...
%         % Enforce sum across t & choices to 1
%         W.Data.RT_pred_pdf = bsxfun(@rdivide, ...
%             W.Data.RT_pred_pdf, sums(W.Data.RT_pred_pdf, [1, 4, 5]));
    ...
    % calc_cost: moved to Fit.D2.Common.Main
...
%                 to_incl(:, [1, end]) = false;
%             otherwise
%                 error('Unknown to_use_easiest_only=%d\n', ...
%                     W.to_use_easiest_only);
%         end
%         
%         cost_sep = nansum(cost_sep);
%         cost = sum(cost_sep(to_incl(:)));
%         
% %         disp(W.th);
% %         disp(cost);
%     end
...
%                 bml.plot.beautify_lim('ax', ax1, 'xy', 'x', ...
%                     'lim_from', 'markers');
%                 bml.plot.beautify_lim('ax', ax1, 'xy', 'y', ...
%                     'lim_from', 'markers');
