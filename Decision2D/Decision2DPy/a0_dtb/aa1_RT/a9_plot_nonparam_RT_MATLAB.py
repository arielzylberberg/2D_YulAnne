...
locfile = localfile.LocalFile(
    '../../Data_2D/Data_2D_Py/a0_dtb/RTNonparamMATLAB')
...
def plot_rt(
    ...
    for i_model, model_name in enumerate(model_names):
        for i_subj, subj in enumerate(subjs):
            ...
            # conds1[1] = (
            #     consts.COHS_COLOR_MANUAL[ix_abs_cond]
            #     * sign_cond
            # )
    ...
    if plot_kind == 'rt_mean':
        ...
        for i_model, model_name in enumerate(model_names):
            ...
            if to_add_legend:
                ...
                for dim in range(n_dim):
                    ...
                    # h._legend_title_box.align = 'center'
                    # ht = h.get_title()
                    # # ht.set_position([0.5, 0])
                    # ht.set_ha('center')
        ...
        # for ext in ['.png', '.pdf']:
        #     file = locfile.get_file_fig('mean_rt', {
        #         'parad': parad,
...
def load_fit(subj: str, parad: str, model_name: str,
    ...
    """
    pth_in = '../../Data_2D/Fit.D2.RT.Td2Tnd.Main'
    nam = (
        # 'export=pred_data+sbj=%s'
        # '+prd=%s+tsk=A+dtk=2+dmr=1+trm=201'
        # '+eor=0+msf=0+ef=0+ec=-0+lf=0'
        # '+eb=10+td=%s+smr=NaN+um=2+us=2'
        #
        'export=pred_data+sbj=%s+prd=%s+tsk=A+dtk=2+dmr=1'
        '+trm=1+eor=0+msf=0+ef=%d+ec=-%d+lf=0+eb=10'
