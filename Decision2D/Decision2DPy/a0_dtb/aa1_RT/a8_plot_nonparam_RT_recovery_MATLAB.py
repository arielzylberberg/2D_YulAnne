...
from a0_dtb.aa1_RT.a6_dtb_2D_recover_RT_nonparam \
...
locfile_out = localfile.LocalFile(
    pth_root='../../Data_2D/Data_2D_Py/a0_dtb/RTNonparamMATLAB',
    cache_dir=''
)
...
files_model_comp = [
    ('eye', 'sbj=S1-S3+prd=RT+tsk=A+dtk=2+dmr=1+trm=1+eor=0+msf=0+ef=%d+ec=-%d+'
    ...
    ('unimanual', 'sbj=S6-S13+prd=unimanual+tsk=A+dtk=2+dmr=1+trm=1+eor=0'
                  '+msf=0+ef=%d+ec=-%d+lf=0+eb=10+td={Ser,'
                  'Par}+smr=NaN+um=2+us=2'
                  '+comp=fval.csv'
                  % (use_easiest_only, use_easiest_only)
    ),
    ('bimanual', 'sbj=S6-S13+prd=bimanual+tsk=A+dtk=2+dmr=1+trm=1+eor=0+msf=0'
                 '+ef=%d+ec=-%d+lf=0+eb=10+td={Ser,Par}+smr=NaN+um=2+us=2'
                 '+comp=fval.csv'
                 % (use_easiest_only, use_easiest_only)
    )
    # ('unimanual', 'sbj=ID16-ID9+prd=unimanual+tsk=A+dtk=2+dmr=1+trm=1+eor=0'
    #               '+msf=0+ef=%d+ec=-%d+lf=0+eb=10+td={Ser,'
    #               'Par}+smr=NaN+um=2+us=2'
    #               '+comp=fval.csv'
    #               % (use_easiest_only, use_easiest_only)
    #  ),
    # ('bimanual', 'sbj=ID16-ID9+prd=bimanual+tsk=A+dtk=2+dmr=1+trm=1+eor=0+msf=0'
    #              '+ef=%d+ec=-%d+lf=0+eb=10+td={Ser,Par}+smr=NaN+um=2+us=2'
    #              '+comp=fval.csv'
    #               % (use_easiest_only, use_easiest_only)
    #  )
...
files_model_recovery = [
    ('eye', 'sbj=S1_tdSer_seed1_ef%d-S3_tdPar_seed1_ef%d+prd=RT+tsk=A+dtk=2'
            '+dmr=1'
            '+trm=1+eor=0+msf=0+ef=%d+ec=-%d+lf=0+eb=10+td={Ser,Par}'
    ...
    ('unimanual', 'sbj=S6_tdSer_seed1_ef%d-S13_tdPar_seed1_ef%d+prd=unimanual'
                  '+tsk=A+dtk=2+dmr=1+trm=1+eor=0+msf=0+ef=%d+ec=-%d+lf=0+eb=10'
    ...
    ('bimanual', 'sbj=S6_tdSer_seed1_ef%d-S13_tdPar_seed1_ef%d+prd=bimanual'
                 ...
                 '+td={Ser,Par}+smr=NaN+um=2+us=2+comp=fval.csv'
                  % tuple([use_easiest_only] * 4)),
    # ('unimanual', 'sbj=ID16_tdSer_seed1_ef%d-ID9_tdPar_seed1_ef%d+prd=unimanual'
    #               '+tsk=A+dtk=2+dmr=1+trm=1+eor=0+msf=0+ef=%d+ec=-%d+lf=0+eb=10'
    #               '+td={Ser,Par}+smr=NaN+um=2+us=2+comp=fval.csv'
    #               % tuple([use_easiest_only] * 4)),
    # ('bimanual', 'sbj=ID16_tdSer_seed1_ef%d-ID9_tdPar_seed1_ef%d+prd=bimanual'
    #              '+tsk=A+dtk=2+dmr=1+trm=1+eor=0+msf=0+ef=%d+ec=-%d+lf=0+eb=10'
    #              '+td={Ser,Par}+smr=NaN+um=2+us=2+comp=fval.csv'
    #               % tuple([use_easiest_only] * 4)),
]
...
def main(base=10):
    ...
    plt.title('Data')
    # for ext in ['.png', '.pdf']:
    #     file = locfile_out.get_file_fig('model_comp', {
