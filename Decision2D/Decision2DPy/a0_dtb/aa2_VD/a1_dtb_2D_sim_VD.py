...
from a0_dtb.aa2_VD import coef_by_dur_vs_odif as coefdur
...
def fun_loss_VD(p_cond_dur__ch_pred: torch.Tensor,
    ...
    #     * torch.log(torch.clamp_min(out.reshape([out.shape[0], -1]),
    #                                 1e-6))
    # ).sum()
...
class Dtb2DVDParallel(Dtb2DVD):
    ...
    def __init__(self,
        ...
        # NOTE: td_offset: Time added to the nomial stimulus duration
        #   Starts from -8/75, for the case where only (9-8)/75 = 1/75 frame is
        #   informative.
        #   Note that this offset determines the offset of the effective
        #   stimulus duration. The slope of the effective stimulus duration is
        #   set by kappa.
        # self.td_offset = torch.tensor([0.] * consts.N_DIM)
        # self.td_offset = ykt.BoundedParameter([0.1] * consts.N_DIM, 0.00, 1e-4)
        self.td_offset = ykt.BoundedParameter([0.] * consts.N_DIM, -8/75, 0.3)
        # self.td_offset = ykt.BoundedParameter([0.] * consts.N_DIM, -0.2, 0.2)
    ...
    def forward(self, ev: torch.Tensor, durs: torch.Tensor
        ...
        # # CHECKED
        # print(unabs_dim_td_cond_ev.shape)
        # unabs_dim_td_cond12_ev = unabs_dim_td_cond_ev.reshape(
        #     unabs_dim_td_cond_ev.shape[:2] + (6, 6)
        #     + (unabs_dim_td_cond_ev.shape[-1],)
        # )
        # p = npy(unabs_dim_td_cond12_ev[0, :, 3, 0, :]).T
        # plt.imshow((p + 1e-12),
        #            extent=[self.t_all[0], self.t_all[-1],
        #                    self.dtb2drt.dtb1ds[0].ev_bin[0],
        #                    self.dtb2drt.dtb1ds[0].ev_bin[-1]])
        # plt.show()
    ...
    def get_parallel_choice(
        ...
        for dim in range(consts.N_DIM):
            ...
            # assert torch.all(durs >= 0)
            # durs_pos = durs > 0
            # itdurs = torch.zeros_like(durs)
        ...
        # p_dim_cond_dur_ch0 = (
        #         torch.stack([
        #             cum_p_dim_cond_td_ch[:, :, itdurs_lb, :],
        #             cum_p_dim_cond_td_ch[:, :, itdurs_ub, :]
        #         ]) * w_lbub_dur[:, None, None, :, None]
        # ).sum(0)
        # for i, durs1 in enumerate([itdurs_lb, itdurs_ub]):
        #     unabs_dim_dur_cond_ev1 = unabs_dim_td_cond_ev[:, durs1, :, :]
        #     ch1 = (
        #         dim_ch_ev[:, None, None, :, :]
        #         @ unabs_dim_dur_cond_ev1.unsqueeze(-1)
        #     ).squeeze(-1)
        #     unabs_dim_dur_cond_ch = (
        #             unabs_dim_dur_cond_ch
        #             + ch1 * w_lbub_dur[i, None, :, None, None]
        #     )
        ...
        for i, ch1 in enumerate(torch.tensor(consts.CHS, dtype=torch.long).T):
            # DEBUGGED: should sum each dim's choice (absorbed OR unabsorbed)
        ...
        # # CHECKED
        # print(unabs_dim_dur_cond_ch.sum(-1))
        # print(p_cond_dur_ch.sum(-1))
        # print('--')
        # # CHECKED
        # plt.plot(npy(unabs_dim_dur_cond_ch.sum((-1, -2)).T))
        # plt.show()
        # plt.imshow(npy(unabs_dim_dur_cond_ch[0, :, 0, :]).T)
        # plt.show()
        # # self.plot_p_ch_by_dur(p_dim_cond_dur_ch0, ch=0)
        # self.plot_p_ch_by_dur(unabs_dim_dur_cond_ch.permute([0, 2, 1, 3]))
        # plt.show()
        #
        # p_cond01_dur_ch01 = npy(p_cond_dur_ch.reshape([6, 6, 10, 2, 2]))
        # plt.plot(p_cond01_dur_ch01[:, 0, :, 1, :].sum((-1,)).T)
        # plt.show()
        #
        # plt.plot(npy(self.dtb2drt.dtb1ds[0].ev_bin), npy(dim_ch_ev[0, :, :]).T)
        # plt.show()
        # print('--')
...
class Dtb2DVDBufSerial(Dtb2DVDParallel):
            ...
            for idur, dur_stim in enumerate(dur_stim_frs):
                ...
                for ich, chs in enumerate(consts.CHS_TENSOR.T):
                    ...
                    for td1st in torch.arange(dur_stim):
                        ...
                        # for td2nd in range(max_td2nd):
                        #     p0[:, ich] = p0[:, ich] + (
                        #         p_dim_cond_td_chDim[dim1st, :, td1st, ch1st]
                        #         * p_dim_cond_td_chDim[dim2nd, :,
                        #                               td2nd, ch2nd]
                        #     )
    ...
    def get_p_cond_dur_chFlat_vectorized(
        ...
        # # Sanity check - passed.
        # # So 1-dimension results are fine.
        # print(
        #     (p_cond_dim_td_chDim.sum((2,3))
        #      + unabs_cond_dim_td_chDim[:, :, -1, :].sum(-1))
        # )
        ...
        for idur, dur_stim in enumerate(dur_stim_frs):
            ...
            for td1st in torch.arange(dur_stim):
                ...
                # td2nds = torch.arange(min([
                #     dur_stim - td1st + dur_buffer_fr,
                #     dur_stim
                # ]))
...
def plot_coefs_dur_odif(
    ...
    for ii_coef, i_coef in enumerate(coefs_to_plot):
        ...
        for i_dim, dim_rel in enumerate(dim_incl):
            ...
            plt.sca(ax)
            # plt2.subplotRC(consts.N_DIM, len(coef_names),
            #                dim_rel + 1, i_coef + 1)
    ...
    for dim_rel in range(consts.N_DIM):
        for idif, dif_irr in enumerate(dif_irrs):
            for i_dur, dur_prct_group in enumerate(dur_prct_groups):
                ...
                jitter1 = 0
                # jitter1 = dx * jitter * (i_dur - (n_dur - 1) / 2)
                kw = consts.get_kw_plot(style, color=cmap(idif),
                                        **dict(kw_plot))
                if style.startswith('data'):
                    plt.plot(x + jitter1, y, **kw)
                    # kw['ecolor'] = kw['mfc']
                    # plt.errorbar(x + jitter, y, yerr=e, **kw)
                else:
                    plt.plot(x + jitter1, y, **kw)
...
def fit_dtb(model: FitVD2D,
    ...
    kw_optim = argsutil.kwdefault(
        ...
        learning_rate=.5,
        patience=100,
        max_epoch=max_epoch,  # CHECKED with small max_epoch
        reduce_lr_after=25,
        reset_lr_after=50,
        thres_patience=1e-4,
        # to_plot_progress=False,  # CHECKED - saves onlt 0.5/3.5 sec per iter
