"""
Modeled after a4_dtb_2D_fit_RT
"""
...
from a0_dtb.aa2_VD import a1_dtb_2D_sim_VD as vd2d
from a0_dtb import a1_dtb_1D_sim as rt1d

# run "tensorboard --logdir runs" in the terminal to see log

locfile = localfile.LocalFile(
    pth_root='../../Data_2D/Data_2D_Py/a0_dtb/VD',
    cache_dir=''
)

i_subjs = (0, 1)  # CHECKED
fix0_pre = [
    # 'tdfree',  # CHECKED fitting td between neg - pos
    # 'tdneg',  # CHECKED fitting td instead of fixing
    # 'tdpos',  # CHECKED fitting td instead of fixing
    # 'tdfix4_0',  # CHECKED tdfix4_0 instead of tdfix4
    'tdfix0',  # CHECKED tdfix0 instead of tdfix4
    # 'tdfix4',  # CHECKED if tdfix4 gives nonconvex w lps1e-12
    'ssq0',
]
fix0_post = [
    # 'basym0_free25',   # 'basym0_free05',
    'basym0_fix',
    'bhalf042_lb01',
    # 'bhalf042_free',
    'diffix',
    ...
    'y0'
    # 'lps1e-6',
    # 'lps1e-8',  # CHECKED lps1e-8 instead of lps0 == lps1e-4
    # 'lps1e-12'  # CHECKED lps1e-12 instead of lps0 == lps1e-4
]
fix0 = fix0_pre + fix0_post

# bufdurs0 = [0., 0.04, 0.08, 0.12, 1.2]
...
bufdurs0 = [0., 0.08, 1.2]
# bufdurs0 = np.array([0.12, 1.2])  # CHECKED with a small number of bufdurs
bufdur_best = 0.08
...
def main():
    ...
    # for buffix in bufdurs0[:3]:
    # for buffix in bufdurs0[3:6]:
    # for buffix in bufdurs0[6:9]:
    # for buffix in bufdurs0[9:12]:
    # for buffix in bufdurs0[::-1]:
        for i_subj in [1]: # range(2):
...
def main_fit(
        ...
        n_fold_valid=1,
        mode_train='all',
        # to_allow_irr_ixn=False,  # CHECKING
        **kwargs
):
    """
    ...
    subdir, dict_subdir = get_subdir(fix_strs, n_fold_valid,
                                     # to_allow_irr_ixn=to_allow_irr_ixn
                                     )
    ...
    model = vd2d.FitVD2D(
        ...
        # dtb2d=vd2d.Dtb2DVDParallel,
        # bound=rt1d.BoundWeibull,
        bound=rt1d.BoundExp,
        dt=dt, nt=nt,
        # to_allow_irr_ixn=to_allow_irr_ixn,
        # **kwargs
    ...
    # params = odict(model.named_parameters())
    # type: # Dict[(str, torch.nn.Parameter)]
    # for k in fix_params:
        if k in fix_params:
            param.requires_grad = False  # DEBUGGED
...
def get_subdir(fix_strs, n_fold_valid=1,
    ...
    # if not to_allow_irr_ixn:
    #     dict_subdir.update({'irixn': False})
    dict_subdir_str = {
        **dict_subdir,
        'fix': ','.join([''] + fix_strs)}
...
def specify_fixs(
    ...
    for fix1 in fix:
        ...
        if fix1 == 'None':
            ...
            state_dict.update({
                'dtb.dur_buffer._ub': 1.5,
                'dtb.dur_buffer._data': [1.5],
            })
            fix_params += [
            ...
            state_dict.update({
                'dtb.dur_buffer._lb': 0.,
                'dtb.dur_buffer._data': [0.],
            })
            fix_params += [
                'dtb.dur_buffer._param'
            ]
            ...
            state_dict.update({
                'dtb.dtb.dtb1ds.0.bound.b_t_half._data': [0.05],
                'dtb.dtb.dtb1ds.1.bound.b_t_half._data': [0.05],
            })
        elif fix1 == 'bhalf0_free':
            state_dict.update({
                'dtb.dtb.dtb1ds.0.bound.b_t_half._data': [0.1],
                'dtb.dtb.dtb1ds.1.bound.b_t_half._data': [0.1],
            })
        elif fix1 == 'bhalf04_free':
            state_dict.update({
                'dtb.dtb.dtb1ds.0.bound.b_t_half._data': [0.4],
                'dtb.dtb.dtb1ds.1.bound.b_t_half._data': [0.4],
                'dtb.dtb.dtb1ds.0.bound.b_t_half._ub': 1.,
                'dtb.dtb.dtb1ds.1.bound.b_t_half._ub': 1.,
            })
        elif fix1 == 'bhalf042_free':
            state_dict.update({
                'dtb.dtb.dtb1ds.0.bound.b_t_half._data': [0.4],
                'dtb.dtb.dtb1ds.1.bound.b_t_half._data': [0.4],
            ...
            state_dict.update({
                'dtb.dtb.dtb1ds.0.bound.b_asymptote._data': [0.1],
                'dtb.dtb.dtb1ds.1.bound.b_asymptote._data': [0.1],
                'dtb.dtb.dtb1ds.0.bound.b_asymptote._ub': 0.2,
                'dtb.dtb.dtb1ds.1.bound.b_asymptote._ub': 0.2,
            })
            # fix_params += [
            #     'dtb.dtb.dtb1ds.0.bound.b_asymptote._param',
            #     'dtb.dtb.dtb1ds.1.bound.b_asymptote._param',
            # ]
        elif fix1 == 'basym0_free':
            state_dict.update({
                'dtb.dtb.dtb1ds.0.bound.b_asymptote._data': [0.1],
                'dtb.dtb.dtb1ds.1.bound.b_asymptote._data': [0.1],
                # 'dtb.dtb.dtb1ds.0.bound.b_asymptote._ub': 0.2,
            ...
            #     'dtb.dtb.dtb1ds.1.bound.b_asymptote._param',
            # ]
        elif fix1 == 'basym0_free05':
            state_dict.update({
                'dtb.dtb.dtb1ds.0.bound.b_asymptote._data': [0.1],
                'dtb.dtb.dtb1ds.1.bound.b_asymptote._data': [0.1],
                'dtb.dtb.dtb1ds.0.bound.b_asymptote._ub': 0.5,
                'dtb.dtb.dtb1ds.1.bound.b_asymptote._ub': 0.5,
            })
            # fix_params += [
            #     'dtb.dtb.dtb1ds.0.bound.b_asymptote._param',
            #     'dtb.dtb.dtb1ds.1.bound.b_asymptote._param',
            # ]
        elif fix1 == 'basym0_free25':
            state_dict.update({
                'dtb.dtb.dtb1ds.0.bound.b_asymptote._data': [0.25],
                'dtb.dtb.dtb1ds.1.bound.b_asymptote._data': [0.25],
                'dtb.dtb.dtb1ds.0.bound.b_asymptote._ub': 0.5,
                'dtb.dtb.dtb1ds.1.bound.b_asymptote._ub': 0.5,
            })
        elif fix1 == 'diffix':
            ...
            state_dict.update({
                'dtb.td_offset._ub': 0.2,
                'dtb.td_offset._lb': -8/75,
        ...
        elif fix1 == 'tst_handfit4':
            state_dict.update({
                'dtb.dtb.dtb1ds.0.bound.b_t_st._data': [0.55],
                'dtb.dtb.dtb1ds.1.bound.b_t_st._data': [0.7]
            })
            fix_params += [
                'dtb.dtb.dtb1ds.0.bound.b_t_st._param',
                'dtb.dtb.dtb1ds.1.bound.b_t_st._param'
            ]
        elif fix1 == 'buf_handfit4':
            state_dict.update({
                'dtb.dur_buffer._data': torch.tensor([0.15]),
            })
            fix_params += [
                'dtb.dur_buffer._param'
            ]
...
# def main_plot_across_models(i_subj=1):
#     models = {}
#     dict_caches = {}
#     ds = {}
#     data = None
#     fixs = ['bufmax', 'None']
#     # fixs = ['buf0', 'bufmax', 'None']  # to crowded
#     kw_plots = {
#         'buf0': {'linestyle': '--'},
#         'None': {'linestyle': '-'},
#         'bufmax': {'linestyle': ':'}
...
#         # fig = None
#
#         axs, fig = plot_coefs_dur_odif_pred_data(
#             data, model,
#             fig=fig, axs=axs,
#             to_plot_data=fix == fixs[-1],
#             kw_plot_model=kw_plots[fix]
#         )
#
#         # plt.show() # CHECKED
#
#
#     dict_cache = {**dict_caches[fixs[0]], 'fixs': ','.join(fixs)}
#     dict_cache.pop('fix')
#     file = locfile.get_file_fig('coefs_dur_odif', dict_cache, subdir=subdir)
...
def main_plot_across_models(
    ...
    for i_subj in range(n_subj):
        for i_model, (name, fix1) in enumerate(fixs):
            ...
            model, data, dict_cache, d, _ = main_fit(
                i_subj, fit_mode='auto', fix=fix, **kwargs
            )  # CHECKING: 'auto' instead of 'load'
            # except RuntimeError:
            #     model = None
            #     data = None
            #     dict_cache = None
    ...
    for i_subj in range(n_subj):
        # plt.sca(axs[-2, i_subj])
        # plt.plot(bufdurs, gofs[:, i_subj], 'k-')
        ...
        # ax0.plot(bufdurs0, gofs[:, i_subj], 'k.-',
        ...
        if i_subj == 0:
            ...
            # plt.ylabel(r'$-\Delta \mathrm{log}~\mathcal{L}$', labelpad=27)
        else:
            ax0.set_yticklabels([])
            ax1.set_yticklabels([])
        ...
        if i_subj == 0:
            ...
            for dim in range(hs1.shape[1] - 1, -1, -1):
                ...
                for dif in range(hs1.shape[2]):
                    ...
                    # for i_kind, kind in enumerate(['data', 'pred']):
                    #     hs.append(hss[0][1, dim, dif][kind])
                odim = 1 - dim
...
def beautify(ax):
    # xticks = np.arange(0, 1.2 + 0.12, 0.12)
    ...
    # if row in [-1, n_row - 1]:
    #     plt2.detach_axis('y', 0)
    #     plt.xticks(
    #         xticks,
    #         ['%g' % v if v in [0, 0.5, 1.] else '' for v in xticks]
    #     )
    # else:
...
def main_plot_models_on_columns(
    ...
    # bufdurs = [0., 0.04, 0.08, 0.12, 0.16, 0.20, 0.36, 0.48, 0.60,
    #            0.96, 1.08, 1.2] # , 1.5]
    # # bufdurs = [0., 0.04, 0.08, 0.12, 0.16, 0.20, 0.36, 0.48, 0.60,
    # #            0.72, 0.84,
    # #            0.96, 1.08, 1.2] # , 1.5]
    ...
    def beautify(row, col, axs):
        xticks = np.linspace(0, 1.3 + 0.1, 0.1)
        # xticks = np.arange(0, 1.2 + 0.12, 0.12)
...
def get_data_2D(i_subj=0, parad='VD',
    ...
    # DEBUGGED: don't subtract 1 from ch here - already done
...
def fit_model_internal(
    ...
    def save_cache(cache, d, best_loss, best_state, subdir):
        ...
        cache.set(d_cache)
        cache.save()
        # localfile.copy2subdir(cache.fullpath, subdir)
    ...
    # CHECKED
    # cache_out = locfile.get_cache('best_output', dict_cache, subdir)
    # cache_out.set({
    #     'out': npy(out),
    #     'inp': [npy(v) for v in inp]
    # })
    # cache_out.save()
    # del cache_out
...
def plot_coefs_dur_odif_pred_data(
    ...
    with torch.no_grad():
        ...
        # ev = ev0.clone()
        dt1 = model.dt
        # durs = torch.arange((durs0[-1] / dt1).long() + 1).clone() * dt1
        # durs = torch.arange(durs0[0] - dt1 * 1 - 1e-2, durs0[-1] + dt1, dt1
        #                     ).clone()
        durs = torch.arange(durs0[0] - dt1 * 0. - 1e-2 * 0, durs0[-1] + dt1,
                            dt1 * 3  # CHECKED
                            ).clone()
        out0 = model(ev_cond_fr_dim_meanvar, durs)
        # out1 = npy(d['out_train_valid'])
        out1 = npy(out0)
        n_cond_dur_ch = npy(data.get_data_by_cond('all')[1])
        # out1 = out1 * n_cond_dur_ch.sum((-1, -2), keepdims=True)
...
kw_handfit = {
    'ID2_handfit2_tdoffset=0': {
        'state_dict': {
            'dtb.dtb.dtb1ds.0.y0._data': torch.tensor([0.04]),
            'dtb.dtb.dtb1ds.0.kb2._data': torch.tensor([50.]),
            'dtb.dtb.dtb1ds.0.bound._data': torch.tensor([1.0]),
            'dtb.dtb.dtb1ds.0.bound_t_st._data': torch.tensor([0.55]),
            'dtb.dtb.dtb1ds.0.bound_t_half._data': torch.tensor([0.05]),
            'dtb.dtb.dtb1ds.0.bound_t_half._lb': 0.01,
            'dtb.dtb.dtb1ds.0.bound_asymptote._data': torch.tensor([0.2]),
            'dtb.dtb.dtb1ds.1.y0._data': torch.tensor([0.0]),
            'dtb.dtb.dtb1ds.1.kb2._data': torch.tensor([35.]),
            'dtb.dtb.dtb1ds.1.bias._data': torch.tensor([-0.01]),
            'dtb.dtb.dtb1ds.1.bound._data': torch.tensor([.8]),
            'dtb.dtb.dtb1ds.1.bound_t_st._data': torch.tensor([0.7]),
            'dtb.dtb.dtb1ds.1.bound_t_half._data': torch.tensor([0.2]),
            'dtb.dtb.dtb1ds.1.bound_asymptote._data': torch.tensor([0.01]),
            'dtb.dtb.kappa_abs_odim._data': torch.tensor([1.6, -0.35]),
            'dtb.dtb.kappa_abs_odim._lb': -5.,
            'dtb.dtb.kappa_abs_odim._ub': 5.,
            'dtb.td_offset._data': torch.tensor([0., 0.]),
            'dtb.td_offset._lb': -0.5,
            'dtb.td_offset._ub': +0.5,
            'dtb.dur_buffer._data': torch.tensor([0.17])
        },
        'fix_params': [
            'dtb.td_offset._param'
        ]
    }, 'ID2_handfit4_tdoffset=nonzero': {
        'state_dict': {
            'dtb.dtb.kappa_abs_odim._data': torch.tensor([1.6, -0.39]),
            'dtb.dtb.kappa_abs_odim._lb': -5.,
            'dtb.dtb.kappa_abs_odim._ub': 5.,
            'dtb.dtb.dtb1ds.0.y0._data': torch.tensor([0.04]),
            'dtb.dtb.dtb1ds.0.kb2._data': torch.tensor([45.]),
            'dtb.dtb.dtb1ds.0.bound._data': torch.tensor([1.05]),
            'dtb.dtb.dtb1ds.0.bound_t_st._data': torch.tensor([0.65]),
            'dtb.dtb.dtb1ds.0.bound_t_half._data': torch.tensor([0.025]),
            'dtb.dtb.dtb1ds.0.bound_t_half._lb': 0.01,
            'dtb.dtb.dtb1ds.0.bound_asymptote._data': torch.tensor([0.5]),
            'dtb.dtb.dtb1ds.1.y0._data': torch.tensor([0.01]),
            'dtb.dtb.dtb1ds.1.bias._data': torch.tensor([-0.01]),
            'dtb.dtb.dtb1ds.1.kb2._data': torch.tensor([35.]),
            'dtb.dtb.dtb1ds.1.bound._data': torch.tensor([.8]),
            'dtb.dtb.dtb1ds.1.bound_t_st._data': torch.tensor([0.7]),
            'dtb.dtb.dtb1ds.1.bound_t_half._data': torch.tensor([0.1]),
            'dtb.dtb.dtb1ds.1.bound_asymptote._data': torch.tensor([0.01]),
            'dtb.td_offset._data': torch.tensor([-0.1, 0.]),
            'dtb.td_offset._lb': -0.5,
            'dtb.td_offset._ub': +0.5,
            'dtb.dur_buffer._data': torch.tensor([0.15])
        }
