...
def get_coefs_mesh_from_histogram_old(
        p_cond_dur_ch: np.ndarray,
        ev_cond_dim: np.ndarray,
        dif_irrs=((2,), (0, 1))
) -> (np.ndarray, np.ndarray, np.ndarray, np.ndarray):
    """
    Old version that ignores irr ixn
    :param p_cond_dur_ch:
    :param ev_cond_dim: [cond, dim]
    :param dif_irrs:
    return: (coef, se_coef, glmres, glmmodel)
    ...
    siz = [n_dim, n_dif, n_dur]
    coef = np.empty([2] + siz)
    se_coef = np.empty([2] + siz)
    glmress = np.empty(siz, dtype=np.object)
    glmmodels = np.empty(siz, dtype=np.object)

    conds = [np.unique(cond) for cond in ev_cond_dim.T]
    n_conds = [len(conds1) for conds1 in conds]
    p_cond_dur_ch = p_cond_dur_ch.reshape(n_conds + [n_dur]
                                          + [consts.N_CH] * 2)
    adconds = [np.unique(np.abs(cond), return_inverse=True)[1]
               for cond in conds]
    ...
    for dim_rel in range(n_dim):
        for idif, dif_irr in enumerate(dif_irrs):
            for idur in range(n_dur):
                dim_irr = consts.get_odim(dim_rel)
                incl = np.isin(adconds[dim_irr], dif_irr)

                if dim_rel == 0:
                    p_cond_ch = p_cond_dur_ch[:, incl, idur, :, :].sum((1, -1))
                else:
                    p_cond_ch = p_cond_dur_ch[incl, :, idur, :, :].sum((0, -2))
                glmmodel = sm.GLM(np.flip(p_cond_ch, -1),
                                  sm.add_constant(conds[dim_rel]),
                                  family=sm.families.Binomial())
                glmres = glmmodel.fit()
                coef[:, dim_rel, idif, idur] = glmres.params
                se_coef[:, dim_rel, idif, idur] = glmres.bse
                glmress[dim_rel, idif, idur] = glmres
                glmmodels[dim_rel, idif, idur] = glmmodel
    return coef, se_coef, glmress, glmmodels
